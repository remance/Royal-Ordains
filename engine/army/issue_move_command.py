...
def issue_move_command(self, target_destination):
    ...
    if (not travelling or travelling["type"] != "retreat") and (travelling or self.current_region != target_destination):
        # cannot issue new move command while in retreat, or move to current region while idle
        ...
        if self.current_region != target_destination:
            ...
            new_travel_route_id = [(item, new_travel_route_id[index + 1]) if
                                   ...
                                   index, item in enumerate(new_travel_route_id) if index + 1 < len(new_travel_route_id)]

            if travelling:
                # travelling already, must check if current path must be added to the new route so army can
                # leave the current route
                current_route_id = self.travelling["id_routes"][0]
                if current_route_id not in new_travel_route_id:
            ...
            current_route_id = self.travelling["id_routes"][0]
            new_travel_route_id = [current_route_id[::-1]]
        ...
        travelling = {"type": command_type, "progress": 0, "destination": target_destination,
                      ...
                      "remain_phase_require": [len(dots) * route_difficulties for index, dots in enumerate(dot_routes)]}
