        ...
        if self.chapter != chapter:  # new chapter load, reload all assets
            ...
            for sub_folder in subdirectories:  # get chapter animation file
                ...
                for file in listdir(part_folder):
                    ...
                    if file_data_name not in self.character_animation_data:
                                            ...
                                            for part_index, part in enumerate(row_part.values()):
                                                if type(part) is list and len(part) > 5:
                                                    ...
                                                    if "effect" in part_name:
                                                        ...
                                                        part_type = part[0]
                                                    if file_data_name not in self.part_sprite_adjust:
            ...
            self.part_sprite_adjust["Item"] = {"Item": {"special": {}}}
            ...
            for item in [f for f in listdir(part_folder) if isfile(join(part_folder, f))]:
                ...
                if item not in self.part_sprite_adjust["Item"]["Item"]["special"]:
                    self.part_sprite_adjust["Item"]["Item"]["special"][item] = {}
                if 0 not in self.part_sprite_adjust["Item"]["Item"]["special"][item]:
                    self.part_sprite_adjust["Item"]["Item"]["special"][item][0] = {}
                if 0 not in self.part_sprite_adjust["Item"]["Item"]["special"][item][0]:
                    self.part_sprite_adjust["Item"]["Item"]["special"][item][0][0] = [1]
            self.char_sprite_chapter["Item"] = int(self.chapter)

            for key, part_data in self.part_sprite_adjust.items():
