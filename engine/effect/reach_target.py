...
def reach_target(self, how=None):
    if self.after_reach_effect:
        ...
        for _ in range(spawn_number):
            ...
            if "reach_effect_spawn_ground" in self.effect_stat["Property"]:  # reach effect spawn with rect bottom on ground
                stat[3] = Default_Ground_Pos * self.screen_scale[1]
                if self.effect_stat["Property"]["reach_effect_spawn_ground"] == "bottom":
                    # spawn position at the bottom of sprite
                    stat[3] -= (self.effect_animation_pool[self.after_reach_effect]["Base"][0][1][1][0]["sprite"][0].get_height() / 2)
                elif self.effect_stat["Property"]["reach_effect_spawn_ground"] == "top":
            ...
            if "no_damage" in self.effect_stat["Property"] or not self.effect_list[self.after_reach_effect]["Damage"]:
