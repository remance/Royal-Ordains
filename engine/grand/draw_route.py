    ...
    if self.player_selected_army:
            ...
            if army.travelling:
                ...
                for route in army.travelling["dot_routes"]:
                    for index, dot in enumerate(route[:-1]):
                        ...
                        start_dot = ((dot[0] * self.map_shown_to_actual_scale_width) - camera_left_bound,
                                     (dot[1] * self.map_shown_to_actual_scale_height) - camera_top_bound)
                        end_dot = ((route[index + 1][0] * self.map_shown_to_actual_scale_width) - camera_left_bound,
                                   (route[index + 1][1] * self.map_shown_to_actual_scale_height) - camera_top_bound)
        ...
        for line_to_draw in lines_to_draws:
            if len(line_to_draw) > 1:
                ...
                draw.lines(self.camera.image, (255, 255, 255), False, line_to_draw, int(12 * self.screen_scale_width))
