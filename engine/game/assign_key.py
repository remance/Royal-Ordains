...
def assign_key(self, key_assign):  # TODO prevent player from input right hat joystick (use only for mouse pos)
    if key_assign not in self.player_key_bind_list[1][self.config["USER"]["control player 1"]].values() or \
            self.player_key_bind[1][self.config["USER"]["control player 1"]][self.input_popup[1]] == key_assign:
        ...
        old_action = tuple(self.player_key_bind_list[1][
                               self.config["USER"]["control player 1"]].values()).index(
            key_assign)
        old_action = \
            tuple(self.player_key_bind_list[1][self.config["USER"]["control player 1"]].keys())[
                old_action]
