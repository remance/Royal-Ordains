...
def menu_keybind(self):
    if self.back_button.event_press or self.esc_press:  # back to start_set menu
        ...
        self.remove_from_ui_updater(self.keybind_text.values(), self.keybind_icon.values())
        self.add_to_ui_updater(self.option_menu_buttons, self.option_menu_sliders.values(), self.value_boxes.values())
        self.add_to_ui_updater(self.option_text_list)
        ...
        for key, value in self.keybind_icon.items():
            if value.event_press:
                self.activate_input_popup(("keybind_input", key),
                                          self.localisation.grab_text(("ui", "key_assign")) + key,
                ...
                self.static_input_box.render_text(self.localisation.grab_text(("ui", "current_key")) + current_key)
