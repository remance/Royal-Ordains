...
def enter_battle(self, animation_data_pool):
    ...
    self.body_parts = {key: BodyPart(self, key) if not any(ext in key for ext in ("special", "weapon")) else BodyPart(self, key, can_hurt=False) for key, value in self.body_parts.items()}
